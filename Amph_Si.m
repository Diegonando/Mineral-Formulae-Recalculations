...
for c=1:m
    Si_N(c,1)=8./Cat(c,1); %Normalize to 15 cations
end

for c=1:m
    N_Cat(c,:)=Cat(c,:).*Si_N(c,:); %normalized cation units
end
...
for c=1:m
    O2total(c,:)=sum(O2(c,:)); %Criteria 2-2: Should be equal to 15 APFU
end
